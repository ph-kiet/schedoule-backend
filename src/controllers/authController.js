...
import { sendWelcomeBusinessOwnerEmail } from "../external-api/postmark/index.js";
...
const register = async (req, res) => {
  try {
    ...
    if (existingUser) {
      return res
        .status(400)
        .json({ errorMsg: `${userDetails.username} is taken!` });
    }
    ...
    res
      .status(200)
      .json({
        message: `Account created successfully! Check you email inbox. `,
      });
  } catch (err) {
    console.log(err);
...
const login = async (req, res) => {
  try {
    ...
    if (!business) {
      return res
        .status(400)
        .json({ errorMsg: `Business ${businessCode} is not found!` });
    }
